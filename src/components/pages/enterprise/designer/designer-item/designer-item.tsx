  ...
  return (
    <li className="border-b border-grey-20 pb-4 pt-[15px]">
      <button
        ...
        onClick={handleOpen}
      >
        <h3 className="text-20 font-medium leading-dense tracking-tight">{question}</h3>
        <ArrowIcon
          className={clsx('ml-auto h-8 w-8 shrink-0 transition-[fill,transform] duration-200', {
            'rotate-0': isOpen,
            'rotate-180': !isOpen,
          })}
          aria-hidden
      ...
      <LazyMotion features={domAnimation}>
        ...
        >
          <div className="flex flex-col pb-1.5 pr-[93px] pt-4 text-16 leading-snug text-grey-70">
            <p>{answer}</p>
          </div>
        </m.div>
      </LazyMotion>
