...
export default async function BlogSubPagesLayout({ children }: { children: ReactNode }) {
  ...
  // Filter categories with no posts
  const filteredCategories = await categories.reduce(async (accumulator, category) => {
    const postsCount = await countPosts({ categoryID: category._id });

    if (postsCount > 0) {
      return [...(await accumulator), category];
    }
    ...
    <>
      <section className="pt-32 lg:pt-[92px] md:pt-[76px] sm:pt-[66px]">
        <div className="container grid max-w-[1794px] grid-cols-14 gap-x-16 gap-y-[33px] lg:gap-x-0 sm:gap-y-[25px]">
          <aside className="top-16 col-span-2 bg-black lg:col-span-full md:sticky md:top-0 md:py-2">
